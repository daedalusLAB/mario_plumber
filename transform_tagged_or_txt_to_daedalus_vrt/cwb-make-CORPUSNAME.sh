...
data="/data/corpora/cqpweb/corpora/test6"
regfile="test6"
name="test6"
registry="/data/corpora/cqpweb/registry"
infile="/tmp/daedalus_vrt/corpus.txt"
threads="10"
...
cwb-encode  -c utf8 -d $data -f $infile -R "$registry/$regfile" -xsB -P pos -P lemma -P lemma_pos \
  ...
  -P is_stop -P head_text -P head_pos -P children -P lefts -P rights -P n_lefts -P n_rights -P ent_type \
  -P ent_iob -P morph -P startsecs -P startcentisecs -P endsecs -P endcentisecs \  
  -S s:0+id+text+reltime -S text:0+id+collection+file+date+year+month+day+time+channel+title+language+program_id \ 
